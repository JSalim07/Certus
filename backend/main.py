# ========================================
# 📦 Importaciones principales y configuración base
# ========================================
...
# ========================================
# 📦 MODELOS Pydantic
# Estos modelos definen la estructura de datos que se reciben en los endpoints.
# ========================================
...
class SubastaCreate(BaseModel):
    ...
    duracion_horas: int = 24   # valor por defecto: 24 horas
...
# Middleware CORS para permitir llamadas desde cualquier origen (útil para frontend separado)
app.add_middleware(
    CORSMiddleware,
...
# Se usa en los endpoints para obtener una sesión de base de datos.
# ========================================
...
class ConnectionManager:
    ...
    """
    def __init__(self):
        # Diccionario: subasta_id -> lista de conexiones WebSocket activas
        self.active_connections: Dict[int, List[WebSocket]] = {}

    async def connect(self, websocket: WebSocket, subasta_id: int):
        """Aceptar conexión y agregarla al grupo de la subasta"""
    ...
    def disconnect(self, websocket: WebSocket, subasta_id: int):
        """Eliminar conexión de la lista cuando se desconecta"""
        if subasta_id in self.active_connections:
            try:
                ...
                # Si ya no quedan conexiones, eliminar la clave
    ...
    async def broadcast(self, subasta_id: int, message: str):
        """Enviar un mensaje a todos los clientes conectados a una subasta"""
        if subasta_id not in self.active_connections:
            return
...
# ========================================
# 🌐 ENDPOINTS HTML
# Estas rutas sirven las páginas frontend desde la carpeta "templates/"
# ========================================
...
# Página de detalle de una subasta específica
...
# ========================================
# 🌐 ENDPOINT DE ESTADO
# Devuelve información básica de la API en formato JSON
# ========================================
@app.get("/status")
def read_status():
    return {
...
def crear_usuario(usuario: UsuarioCreate, db: Session = Depends(get_db)):
    """Registrar un nuevo usuario validando duplicados"""
...
async def crear_puja(puja: PujaCreate, db: Session = Depends(get_db)):
    """Registrar una nueva puja y notificar a los usuarios conectados"""
    ...
    if puja.monto <= subasta.precio_actual:
        raise HTTPException(status_code=400, detail=f"La puja debe ser mayor a ${subasta.precio_actual:.2f}")

    usuario = db.query(Usuario).filter(Usuario.id == puja.usuario_id).first()
    if not usuario:
        raise HTTPException(status_code=404, detail="Usuario no encontrado")

    nueva_puja = Puja(monto=puja.monto, usuario_id=puja.usuario_id, subasta_id=puja.subasta_id)
